...
#for file in *.pem; do curl -v -H 'User-Agent:' -H 'Accept:' -F name=fileName -F fileName="@$file" "http://192.168.100.5/cgi-bin/upload.cgi" ; done
#thttpd -p 8080 -d /tmp -c /*.cgi -u root

# Check if arguments flags include "--patch"
if [ $PATCHED ]; then
	echo "Patched firmware, using modified upload.cgi..."
	curl -v -H 'User-Agent:' -H 'Accept:' -F name=fileName -F fileName=@$1 "http://192.168.100.5:8080/upload.cgi"
else
	echo "Original firmware, using original upload.cgi..."
	curl -v -H 'User-Agent:' -H 'Accept:' -F name=fileName -F fileName=@$FILEPATH "http://192.168.100.5/cgi-bin/upload.cgi"
fi
