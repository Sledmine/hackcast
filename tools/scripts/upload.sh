...
#for file in *.pem; do curl -v -H 'User-Agent:' -H 'Accept:' -F name=fileName -F fileName="@$file" "http://192.168.100.11:8080/upload.cgi" ; done
#thttpd -p 8080 -d /tmp -c /*.cgi -u root

# Check if arguments flags include "--patch"
if [ $PATCHED ]; then
	echo "Patched firmware, using modified upload.cgi..."
	curl -v -H 'User-Agent:' -H 'Accept:' -F name=fileName -F fileName=@$FILEPATH "http://192.168.100.11/cgi-bin/upload.cgi"
else
	echo "Original firmware, using original upload.cgi..."
	cp $FILEPATH "$FILEPATH.pem"
	curl -v -H 'User-Agent:' -H 'Accept:' -F name=fileName -F fileName=@$FILEPATH.pem "http://192.168.100.11/cgi-bin/upload.cgi"
	rm "$FILEPATH.pem"
fi
