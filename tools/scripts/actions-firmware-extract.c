/* actions-firmware-extract - extract firmwares like the one for Acer K330, AM8268B, etc.
*
...
static int parse_section(uint8_t **bufferp, struct actions_firmware_section *section)
{
	...
	// TODO Need to modify to account this issue: https://github.com/c3c/miracast/issues/4#issuecomment-434739325
	section->start_address = get_le32(bufferp) + 0x2000;
...
int main(int argc, char *argv[])
{
	...
	// Read arg flags and look for --dump
	...
	if (dump)
	{
		printf("Dumping sections to %s\n", path_prefix);
		while ((err = parse_section(&buffer_iterator, &section)) == 0)
		{
			print_section(&section);
			i++;

			if (strncmp((char *)section.name, "YOLO", 16) == 0)
			{
